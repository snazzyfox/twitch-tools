<template>
  <q-page class="q-ma-xl q-gutter-md page-width">
    ...
    <q-form ref="form">
      ...
      <slider-field v-model="config.player.autohide.delaySeconds" :min="1" :max="30" :step="1" unit="seconds"
      ...
      <twitch-signin hint="Sign in if you want Twitch related functionalities" v-model="config.twitchBot.twitchAuth"
        @update:model-value="handleTwitchAuthChange" />

      <twitch-channel-selection v-model="config.twitchBot.channelName" label="Twitch Channel"
        hint="Twitch channel to listen for commands in" />
      <q-input v-model="config.twitchBot.commands.info.text" label="Command to request song info in chat"
        hint="Viewers can use this command in chat get a reply with the name and link to the playing song."
        :disable="!config.twitchBot.twitchAuth?.token || !config.twitchBot.commands.info.enabled" prefix="!">
        <template #before>
          <q-toggle v-model="config.twitchBot.commands.info.enabled" :disable="!config.twitchBot.twitchAuth?.token" />
        </template>
      </q-input>
      <q-input v-model="config.twitchBot.commands.show.text" label="Command to show overlay if hidden"
        ...
        :disable="!config.twitchBot.twitchAuth?.token ||
          ...
          " prefix="!">
        <template #before>
          <q-toggle v-model="config.twitchBot.commands.show.enabled"
      ...
      <div class="q-gutter-sm row">
        <q-toggle :model-value="true" label="Track Name" @click="$q.notify('You can\'t turn off the track name.')" />
        <color-picker-input v-model="config.player.trackName.color" label="Color" />
        <slider-field v-model="config.player.trackName.size" :min="1" :max="30" :step="0.1" unit="%" label="Font Size" />
      </div>

      <div class="q-gutter-sm row">
        <q-toggle v-model="config.player.artistName.enabled" label="Artist Name" />
        <color-picker-input v-model="config.player.artistName.color" label="Color"
          :disable="!config.player.artistName.enabled" />
        <slider-field v-model="config.player.artistName.size" :disable="!config.player.artistName.enabled" :min="1"
          :max="30" :step="0.1" unit="%" label="Font Size" />
      </div>

      <div class="q-gutter-sm row">
        <q-toggle v-model="config.player.albumName.enabled" label="Album Name" />
        <color-picker-input v-model="config.player.albumName.color" label="Color"
          :disable="!config.player.albumName.enabled" />
        <slider-field v-model="config.player.albumName.size" :disable="!config.player.albumName.enabled" :min="1"
          :max="30" :step="0.1" unit="%" label="Font Size" />
      </div>

      <div class="q-gutter-sm row">
        <q-toggle v-model="config.player.scroll.enabled" label="Scroll Long Text" />
        <slider-field v-model="config.player.scroll.speed" :min="0.1" :max="5" :step="0.1" unit="px/s" label="Speed"
          :disable="!config.player.scroll.enabled" />
      </div>

      <div class="q-gutter-sm row">
        <q-toggle v-model="config.player.progress.bar" label="Show Progress Bar" />
        <q-toggle v-model="config.player.progress.elapsed" label="Show Elapsed Time"
          :disable="!config.player.progress.bar" />
        <q-toggle v-model="config.player.progress.length" label="Show Song Length"
          :disable="!config.player.progress.bar" />
        <color-picker-input v-model="config.player.progress.color" label="Progress Row Color"
          :disable="!config.player.progress.bar" />
      </div>
    </q-form>
